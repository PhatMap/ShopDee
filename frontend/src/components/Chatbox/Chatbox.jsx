...
const Chatbox = ({ onClose }) => {
  ...
  const { chats, chatDetails, usersInChats, loading, error, searchResults } =
  ...
  const LIKE_IMAGE_URL =
    "https://res.cloudinary.com/dyeelociz/image/upload/v1726151496/e66c3a34-f651-46d2-ad36-2602581a469c.png";
  ...
  const handleSendMessage = async () => {
    if ((message.trim() || chatImages.length > 0) && selectedChat) {
      ...
      if (chatImages.length > 0) {
          ...
          chatImages.map(async (image) => {
            ...
            try {
              ...
              if (
                ...
                console.error(
                  "Unexpected result structure from uploadChatImages:",
                  result
                );
                return null;
              }
        ...
        cloudinaryChatImages = cloudinaryChatImages
          .flat()
          .filter((img) => img !== null);
      }
  ...
  const currentChatParticipant = chatDetails?.participants?.find(
    (p) => p._id !== user._id
  );
  ...
  useEffect(() => {
    if (chatDetails && chatDetails.participants) {
      const currentParticipant = chatDetails.participants.find(
        (p) => p._id !== user._id
      );
  ...
  const handleChatImageRemove = (index) => {
    setChatImagesPreview((prevImages) =>
            ...
            {showSearchResults ? (
              ...
              usersInChats.map((chatUser) => (
                <div
                  key={chatUser._id}
                  className={`chat-item ${
                    selectedChat === chatUser.chatId ? "selected" : ""
                  }`}
                      ...
                      <div className="message-content">
                        <div className="message-content">
                          <div className="message-bubble">
                            ...
                            {msg.icon && (
                              <img src={msg.icon} className="message-icon" />
                            )}
                          </div>
                                ...
                                <div className="icon-picker">
                                  {iconList.map((icon) => (
                                    <img
                                      ...
                                      onClick={() =>
                                        handleIconClick(msg._id, icon.url)
                                      }
                                    />
                                  ))}
                                  {msg.icon && (
                                    <img
                                      ...
                                      className="emoji-icon"
                                      onClick={() =>
                                        handleIconClick(msg._id, "remove")
                                      }
                                    />
                                  )}
                                </div>
